...
MAE_BASE="https://www.diplomatie.gouv.fr/fr/information-par-pays/"
ALERT_LEVELS=[("formellement déconseillé","formellement_deconseille","red"),("déconseillé sauf raison impérative","deconseille_sauf_ri","orange"),("déconseillé sauf raison","deconseille_sauf_ri","orange"),("déconseillé","deconseille","orange"),("vigilance renforcée","vigilance_renforcee","yellow"),("vigilance normale","vigilance_normale","green")]
...
def fetch_flightaware(db):
    ...
    # Guard horaire : uniquement entre 6h et 18h heure de Paris
    try:
        from zoneinfo import ZoneInfo
    except ImportError:
        from backports.zoneinfo import ZoneInfo
    paris_now=datetime.now(ZoneInfo("Europe/Paris"))
    if paris_now.hour<6 or paris_now.hour>=18:
...
def scrape_mae():
    ...
    for ck,slug in MAE_SLUGS.items():
        url_secu=f"{MAE_BASE}{slug}/conseils-aux-voyageurs-securite"
        url_alerte=f"{MAE_BASE}{slug}/dernieres-minutes-et-alertes"
        url_display=url_secu  # URL affichée dans le dashboard
        try:
            # Scraper les deux pages et combiner le texte
            all_paras=[]
            all_full_text=""
            for page_url,page_label in [(url_secu,"sécurité"),(url_alerte,"alertes")]:
                try:
                    r=requests.get(page_url,timeout=15,headers=HDR)
                    print(f"  {ck} ({page_label}): HTTP {r.status_code}",flush=True)
                    if r.status_code==200:
                        soup=BeautifulSoup(r.content,"html.parser")
                        all_full_text+=" "+soup.get_text().replace('\n',' ')
                        paras=[p.get_text(strip=True) for p in soup.find_all("p") if len(p.get_text(strip=True))>=15]
                        all_paras.extend(paras)
                except Exception as pe:
                    print(f"  ⚠ {ck} ({page_label}): {str(pe)[:150]}",flush=True)
                time.sleep(0.5)

            if not all_paras and not all_full_text:
                res[ck]=_mfb(ck,url_display,"Aucune page accessible"); continue

            full_text=all_full_text.lower()
            rel=[t for t in all_paras if any(k in t.lower() for k in ["déconseillé","vigilance","quitter","se rendre","recommandé","risque","frappes","prudence","sécurité","zone","éviter","déplacement","frontière","aéroport"]) and not any(g in t.lower() for g in MAE_GENERIC)]
            rt=" ".join(rel).lower()
            found=[(lt,cd,co) for lt,cd,co in ALERT_LEVELS if lt in rt]
            # Fallback : chercher dans le texte complet si rien trouvé dans les paragraphes filtrés
            if not found:
            ...
            upd=""; um=re.search(r'Dernière mise à jour[^\d]*(\d{1,2}\s+\w+\s+\d{4})',all_full_text)
            if um: upd=um.group(1).strip()
            res[ck]={"country":ck,"label":MAE_LABELS.get(ck,ck),"level":ll,"level_code":lc,"color":lcl,"is_partial":ip,"summary":ss,"full_content":fc,"url":url_display,"last_update_mae":upd,"conseil_tourisme":"","last_scraped":datetime.now(timezone.utc).isoformat()}
            ...
            res[ck]=_mfb(ck,url_display,str(e)[:200])
